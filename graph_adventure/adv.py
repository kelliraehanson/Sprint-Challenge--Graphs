...
world.printRooms() # You can uncomment this to see the map
...
# When you player enters a room, she will note the exits and add a "?" for a temporary room num.
...
# FILL THIS IN
traversalPath = []
mygraph = {} # Graph keeps track of all the rooms the player visits
move_backwards_path = [] # Empty for player to move on a backwards path
current_room_exits = player.currentRoom.getExits() # Using the player's current room's getExit() in room.py to find the exits 

mygraph[player.currentRoom.id] =  {
    current_room_exits[i]: 
    "?" for i in range(0,len(current_room_exits))
    } # When you player enters a room, she will note the exits and add a "?" for a temporary placeholder room number.
print(f"This is mygraph: {mygraph}") # Print mygraph

while len(roomGraph) - 1 > len(mygraph): # While the total rooms in roomGraph is greater than the length of mygraph dictionary
    if player.currentRoom.id not in mygraph: # If the player's currentRoom is not in the dictionary of mygraph
        current_room_exits = player.currentRoom.getExits() 
        # Add current room and the exits with the "?" to the mygraph dictionary
        mygraph[player.currentRoom.id] = {
            current_room_exits[i]: 
            "?" for i in range(0,len(current_room_exits))
            } # When you player enters a room, she will note the exits and add a "?" for a temporary placeholder room number.

        # Set the direction the player came from to that room using getroomindirection and move_backwards_path 
        mygraph[player.currentRoom.id][move_backwards_path[-1]] = player.currentRoom.getRoomInDirection(move_backwards_path[-1])

    # Find the rest of the exits the player stil needs to explore and add them to the list
    each_exit = []
    for x_exit, room in mygraph[player.currentRoom.id].items():
        if room == "?":
            each_exit.append(x_exit)

    # While the player has no more rooms to explore, but there are still elements in move_backwards_path
    while len(each_exit) == 0 and len(move_backwards_path) > 0:
        move_backwards_other = move_backwards_path.pop() # pop (remove) move_backwards_path to use as player's next move
        traversalPath.append(move_backwards_other)  # Add move_backwards_other to the traveralPath
        player.travel(move_backwards_other)  # The Player moves in that direction


        all_new_exits = [] # Find all new exits and add them to list
        for x_exit, room in mygraph[player.currentRoom.id].items():
            if room == "?":
                all_new_exits.append(x_exit)
        each_exit = all_new_exits # Reset exits

# Set the room's last each_exit to getRoomInDirection
    mygraph[player.currentRoom.id][each_exit[-1]] = player.currentRoom.getRoomInDirection(each_exit[-1])

# Set the player's next direction_move as last each_exit in the list
    direction_move = each_exit.pop()

# When a player hits a dead end they turn around in the opposite direction of their move
    opposite_directions = { 
    "n": "s", 
    "s": "n", 
    "e": "w", 
    ...
    # Add the player's new direction_move to the path
    move_backwards_path.append(opposite_directions[direction_move])
    player.travel(direction_move) # Player moves in that direction
    traversalPath.append(direction_move) # Add direction_move to the traveralPath


print(f"\n** These are the current room exits {current_room_exits} **\n")
print(f"** This is the traversal path: {traversalPath} **\n")
print(f"\n** This is the player's current room: {player.currentRoom}\n **\n")
